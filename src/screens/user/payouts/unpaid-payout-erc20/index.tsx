...
import type { DraftPayoutERC20, PendingPayoutERC20 } from '@/types/user-payouts'
import { ConnectWallet } from './connect-wallet'

const isPendingPayout = (
  payout: DraftPayoutERC20 | PendingPayoutERC20
): payout is PendingPayoutERC20 =>
  (payout as PendingPayoutERC20).amountInTokens !== undefined &&
  (payout as PendingPayoutERC20).transactions !== undefined
...
export default function Screen({ payout }: Props) {
  ...
  const { config: withdrawTokensConfig, error: withdrawTokensPrepareError } =
    usePrepareContractWrite({
      enabled: isClaimEnabled,
      abi: sellerMarketplaceABI,
      functionName: 'withdrawTokensAmount',
      address: payout.sellerMarketplaceSmartContractAddress,
      chainId: payout.networkChainId,
      args: [payout.tokenSmartContractAddress, BigInt(payout.amountInTokens)],
      account: accountAddress,
      gas: 1000000n,
    })

  const {
    write: writeWithdrawTokens,
    error: withdrawTokensError,
    isLoading: withdrawProcessing,
    isSuccess: withdrawProcessed,
  } = useContractWrite({
    ...withdrawTokensConfig,
    onSuccess(data) {
      setTxHash(data.hash)
    },
  })
                    ...
                    <>
                      {isEnoughMoney ? (
                        <>
                          ...
                          {withdrawTokensPrepareError && (
                            <ErrorMessage
                              header="Unable to prepare withdrawal"
                              content={withdrawTokensPrepareError.message}
                            />
                          )}
                          ...
                          {withdrawTokensError && (
                            <ErrorMessage
                              ...
                              content={withdrawTokensError.message}
                            />
                          )}
