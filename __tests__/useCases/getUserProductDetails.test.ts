...
import {
  ...
  UserDoesNotExist,
  ProductDoesNotExist,
} from '@/useCases/getUserProductDetails'
...
describe('getUserProductDetails', () => {
  const request = {
    userId: uuidv4(),
  ...
  describe('when userId is not a valid uuid', () => {
    it('returns error', async () => {
      ...
      try {
        const result = await getUserProductDetails({
          ...request,
          userId: 'not-an-uuid',
        })
  ...
  describe('when everything is good', () => {
    ...
    it('returns user product', async () => {
      const userProductDetails = await getUserProductDetails({
        userId: user.id,
        userProductId: userProduct.id,
      })

      expect(userProductDetails.id).toEqual(userProduct.id)
      expect(userProductDetails.productCategoryTitle).toEqual('Category')
      expect(userProductDetails.slug).toEqual('product-title')
      expect(userProductDetails.title).toEqual('Product Title')
      expect(userProductDetails.priceFormatted).toEqual('1233.09391929 COIN')
    })
  })
})
